...
function Models() {
  ...
  const addModel = (e) => {
    ...
    axios({
      ...
      .catch(() => {
        toast.error('Model could not be added');
      });
  };
  ...
  return (
    <>
      <button
        className="text-[20px] font-bold bg-[#01002b] text-white px-5 py-3 rounded-[15px] mt-2"
      ...
      <div className="grid grid-cols-3 pt-5 gap-5">
        {loading ? (
          ...
          models.map((model) => (
            <div
              className="grid grid-cols-1 gap-5 p-5 bg-[#939396] rounded-[20px] text-white"
              key={model.id}
            >
              <h1>{model.name}</h1>
